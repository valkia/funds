...
export default {
  ...
  methods: {
    ...
    fundDetail(val) {
      this.sltFund = val;
      console.log(`fundDetail：`,this.sltFund)
      this.detailShadow = true;
    ...
    getData(type) {
      ...
      this.fundListM.map(async (res) => {
        ...
        let data = {
          ...
          name: val.instId,
          jzrq: val.PDATE,//截止日期
          dwjz: isNaN(val.last) ? null : val.last,
          gsz: isNaN(val.last) ? null : val.last, // 估算净值
          gszzl: (val.last>val.sodUtc0?1:-1)*(val.last/val.sodUtc0).toFixed(2), // 涨跌幅
          gztime: val.GZTIME,
        };
